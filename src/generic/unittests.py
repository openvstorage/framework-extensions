...
class UnitTest(object):
    ...
    def list_cases_for_file(self, file_path):
        ...
        if not file_path.endswith('.py'):
            file_path += '.py'
        self._gather_test_info(directories=os.path.dirname(file_path))
    ...
    def list_tests(self, directories=None, print_tests=False, silent_invalid_modules=False):
        ...
        :rtype: list
        """
        self._gather_test_info(directories=directories, silent_invalid_modules=silent_invalid_modules)
        tests = sorted(key for key in self._test_info if ':' not in key and '.' not in key)
    ...
    def run_tests(self, tests=None, add_averages=False, path=None):
        ...
        if tests is None:  # Put all test files and their classes in custom dict
            ...
            failed_modules = self._invalid_test_modules
        else:  # Validate the specified tests
            self._gather_test_info(silent_invalid_modules=True)
            errors = []
            if isinstance(tests, str):
                tests = tests.split(',')
            if not isinstance(tests, list):
                raise ValueError('Tests should be a list')
            sorted_tests = sorted(self._test_info.keys())
            tests_to_execute = []
            for test in tests:
                found_tests = False
        ...
        for test in tests_to_execute:
            ...
            text_string = '# Processing {0} {1} #'.format(self._test_info[test]['use_case'], test)
            print '\n\n\n{0}\n{1}\n{0}\n'.format(len(text_string) * '#', text_string)
            tests_to_run = self._test_info[test]['tests']
            ...
            test_results.append('    - DURATION: {0}'.format(self._sec_to_readable(duration)))
            test_results.append('    - {0}: {1}'.format(self._SUCCESS, test_amount - len(result.errors) - len(result.failures)))
            ...
            if len(result.failures) > 0:
                ...
                test_results.append('    - {0}: {1}'.format(self._FAILURE, len(result.failures)))
            ...
            if len(result.errors) > 0:
                ...
                test_results.append('    - {0}: {1}'.format(self._ERROR, len(result.errors)))
        ...
        if len(tests_to_execute) > 1 or len(failed_modules) > 1:
            ...
            if len(tests_to_execute) > 1:
                ...
                test_results.append('  - Total duration: {0}'.format(self._sec_to_readable(time.time() - start_all)))
                test_results.append('    - {0}: {1} / {2} ({3:.2f} %)'.format(self._SUCCESS, total_success, int(total_tests), total_success / total_tests * 100))
                if total_failure > 0:
                    test_results.append('    - {0}: {1} / {2} ({3:.2f} %)'.format(self._FAILURE, total_failure, int(total_tests), total_failure / total_tests * 100))
                if total_error > 0:
                    test_results.append('    - {0}: {1} / {2} ({3:.2f} %)'.format(self._ERROR, total_error, int(total_tests), total_error / total_tests * 100))
            if len(failed_modules) > 1:
                test_results.append('  - {0}: {1} invalid test modules'.format(self._ERROR, len(failed_modules)))
            test_results.append('')
            ...
            for index, line in enumerate(test_results[:]):
                if line in averages:
                    ...
                    if average_duration > 1:
                        new_line = '{0} - {1} per test)'.format(line[:-1], self._sec_to_readable(average_duration, 2))
